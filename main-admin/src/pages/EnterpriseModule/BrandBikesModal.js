...
const BrandBikesModal = ({ brand, collectionName, onClose }) => {
  ...
  return (
    <div className="module-modal-backdrop">
      <div className="module-modal" style={{ maxWidth: '800px', width: '90%' }}>
        ...
        {error && <div className="enterprise-toast error" style={{ position: 'relative', margin: '10px 24px', transform: 'none' }}><span>{error}</span></div>}
        {success && <div className="enterprise-toast" style={{ position: 'relative', margin: '10px 24px', transform: 'none' }}><span>{success}</span></div>}

        <div className="modal-grid" style={{ display: 'block', maxHeight: '60vh', overflowY: 'auto', padding: '0 24px' }}>
          {bikes.length === 0 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <label className="field">
                      ...
                      {(bike.imageUrl || bikeImageFiles[index]) && <img className="module-image-preview" src={bikeImageFiles[index] ? URL.createObjectURL(bikeImageFiles[index]) : bike.imageUrl} alt={`${bike.name || 'Bike'} preview`} />}
                      <input 
                        type="file"
                        accept="image/*"
          ...
          <button 
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#0f766e', background: 'none', border: 'none', cursor: 'pointer', padding: '8px 0', fontWeight: '500' }}
          >
            <Plus size={16} /> Add Bike
          </button>
        </div>

        <div className="modal-actions" style={{ marginTop: '0', borderTop: '1px solid #e2e8f0' }}>
          <button type="button" className="export-btn" onClick={onClose} disabled={loading}>Cancel</button>
          <button type="button" className="quick-add-btn modal-save-button" onClick={handleSave} disabled={loading}>
