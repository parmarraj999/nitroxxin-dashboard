...
import { bikeBrands, brands, categoryCatalog, getVariantTemplate } from './productFormConfig';
...
const AddProduct = () => {
  ...
  const config = categoryCatalog[form.category] || categoryCatalog.Helmets;
  ...
  React.useEffect(() => {
    const nextConfig = categoryCatalog[form.category];
    if (!nextConfig) return;
    setForm((current) => ({
      ...current,
      subCategory: nextConfig.subcategories.includes(current.subCategory) ? current.subCategory : nextConfig.subcategories[0],
      productType: nextConfig.productTypes.includes(current.productType) ? current.productType : nextConfig.productTypes[0]
    }));
  }, [form.category]);
  ...
  return (
    <div className="add-product-page">
      ...
      {lastSaved && <div className="autosave-strip"><CheckCircle2 size={16} /> Last saved at {lastSaved}</div>}
          ...
          {activeStep === 'category' && (
            <section className="form-section card">
              ...
              <div className="form-grid three">
                ...
                <div className="form-group"><label>Brand *</label><select value={form.brand} onChange={(event) => setField('brand', event.target.value)}>{brands.map((brand) => <option key={brand}>{brand}</option>)}</select></div>
          ...
          {activeStep === 'content' && (
            <section className="form-section card">
              ...
              <div className="form-grid two">
                ...
                <div className="form-group"><label>SEO Title</label><input value={form.seoTitle} onChange={(event) => setField('seoTitle', event.target.value)} /></div>
                <div className="form-group"><label>Slug</label><input value={form.slug} onChange={(event) => setField('slug', event.target.value)} /></div>
                <div className="form-group full"><label>SEO Description</label><textarea value={form.seoDescription} onChange={(event) => setField('seoDescription', event.target.value)} /></div>
            ...
            <section className="form-section card">
              ...
              <div className="media-uploader">
                <div className="upload-box large" onClick={() => fileInputRef.current?.click()}>
                  <Upload size={28} />
                  <strong>{uploading ? 'Uploading...' : 'Upload thumbnail / main product image'}</strong>
                  <span>Images and PDF certificates are stored with Firebase Storage.</span>
                  <input ref={fileInputRef} type="file" accept="image/*,application/pdf" style={{ display: 'none' }} onChange={uploadPrimaryImage} />
                </div>
                <div className="media-preview" style={{ backgroundImage: form.media.primaryImage ? `url(${form.media.primaryImage})` : undefined }}>
                  {!form.media.primaryImage && <ImagePlus size={40} />}
                </div>
              </div>
              <div className="form-grid two">
